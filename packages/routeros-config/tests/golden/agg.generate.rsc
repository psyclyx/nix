...
# ── Routes ──
/ip route
add disabled=no dst-address=0.0.0.0/0 gateway=10.0.252.1 comment="iyr over core transit"

# ── DHCP relay ──
/ip dhcp-relay
add name=relay-lab interface=vlan210 dhcp-server=10.0.252.1 local-address=10.0.210.1 disabled=no
add name=relay-main interface=vlan10 dhcp-server=10.0.252.1 local-address=10.0.10.1 disabled=no
add name=relay-storage interface=vlan200 dhcp-server=10.0.252.1 local-address=10.0.200.1 disabled=no
