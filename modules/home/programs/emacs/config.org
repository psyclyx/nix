...
#+begin_src emacs-lisp
  (use-package eglot
    ...
    :config
    (defvar my/eglot-init-map (make-sparse-keymap))
    (defvar my/eglot-map (make-sparse-keymap))
    
    (general-def
      :keymaps 'my/eglot-init-map
      "l" 'eglot
      "s" 'eglot-shutdown)
    
    (general-def
      :keymaps 'my/eglot-map
      "r" 'eglot-reconnect)

    (my-leader-def "l" (cons "lsp*" my/eglot-init-map))
    (my-leader-def :keymaps 'eglot-mode-map "l" (cons "lsp" my/eglot-map)))
#+end_src
